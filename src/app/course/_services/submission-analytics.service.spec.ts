...
describe('SubmissionAnalyticsService', () => {
    ...
    it('should return all submission analytics'), () =>{
        submissionAnalyticsService.getAllSubmissionAnalytics().subscribe((submissionAnalytics) => {
            expect(submissionAnalytics.results).toEqual(MOCK_SUBMISSION_ANALYTICSS);
        });
        const request = httpMock.expectOne('http://localhost:8000/api/submission-analytics');
